...
- [Group-wise Tokenizer](#group-wise-tokenizer)
- [Auto-Split Tokenizer](#auto-split-tokenizer)
- [5.1.1 S-token 为什么共享参数](#511-s-token-为什么共享参数)
- [5.1.2 NS-token 为什么使用 token-specific 参数](#512-ns-token-为什么使用-token-specific-参数)
- [5.1.3 Q/K/V 参数策略怎么理解](#513-qkv-参数策略怎么理解)
- [5.1.4 FFN 参数策略怎么理解](#514-ffn-参数策略怎么理解)
- [5.1.5 这个策略的取舍](#515-这个策略的取舍)
- [6.3.1 FlashAttention-2](#631-flashattention-2)
- [6.3.2 BF16/FP16 混合精度](#632-bf16fp16-混合精度)
- [6.3.3 Activation Re-computation](#633-activation-re-computation)
- [6.3.4 KV Cache](#634-kv-cache)
- [6.3.5 四个优化放在一起看](#635-四个优化放在一起看)
- [7.2.1 Base model: DCNv2 + DIN](#721-base-model-dcnv2-din)
- [7.2.2 Feature-interaction: 只替换特征交互模块](#722-feature-interaction-只替换特征交互模块)
- [7.2.3 Sequence-modeling: 只替换序列建模模块](#723-sequence-modeling-只替换序列建模模块)
- [7.2.4 Unified framework: OneTrans](#724-unified-framework-onetrans)
- [7.2.5 这张表应该怎么读](#725-这张表应该怎么读)

# 目录

- [# 2.1 工业推荐排序中的典型建模范式](#21-工业推荐排序中的典型建模范式)
- [# 2.2 传统范式的局限](#22-传统范式的局限)
- [# 4.1 序列特征：S-tokens](#41-序列特征s-tokens)
  - [# timestamp-aware fusion](#timestamp-aware-fusion)
  - [# timestamp-agnostic fusion](#timestamp-agnostic-fusion)
- [# 4.2 非序列特征：NS-tokens](#42-非序列特征ns-tokens)
  - [# Group-wise Tokenizer](#group-wise-tokenizer)
  - [# Auto-Split Tokenizer](#auto-split-tokenizer)
- [# 5.1 S-token 与 NS-token 使用不同参数策略](#51-s-token-与-ns-token-使用不同参数策略)
  - [# 5.1.1 S-token 为什么共享参数](#511-s-token-为什么共享参数)
  - [# 5.1.2 NS-token 为什么使用 token-specific 参数](#512-ns-token-为什么使用-token-specific-参数)
  - [# 5.1.3 Q/K/V 参数策略怎么理解](#513-qkv-参数策略怎么理解)
  - [# 5.1.4 FFN 参数策略怎么理解](#514-ffn-参数策略怎么理解)
  - [# 5.1.5 这个策略的取舍](#515-这个策略的取舍)
- [# 5.2 Causal Attention](#52-causal-attention)
- [# 5.3 位置编码与顺序信息](#53-位置编码与顺序信息)
- [# 5.4 RMSNorm Pre-Norm](#54-rmsnorm-pre-norm)
- [# 6.1 Pyramid Stack](#61-pyramid-stack)
- [# 6.2 Cross-request KV Caching](#62-cross-request-kv-caching)
- [# 6.3 复用 LLM 训练和推理优化](#63-复用-llm-训练和推理优化)
  - [# 6.3.1 FlashAttention-2](#631-flashattention-2)
  - [# 6.3.2 BF16/FP16 混合精度](#632-bf16fp16-混合精度)
  - [# 6.3.3 Activation Re-computation](#633-activation-re-computation)
  - [# 6.3.4 KV Cache](#634-kv-cache)
...
- [# 7.2 对比模型架构怎么理解](#72-对比模型架构怎么理解)
  ...
  - [# 7.2.5 这张表应该怎么读](#725-这张表应该怎么读)
- [# 7.3 离线效果](#73-离线效果)
- [# 7.4 在线 A/B 效果](#74-在线-ab-效果)
- [# 8.1 Auto-Split Tokenizer 优于 Group-wise Tokenizer](#81-auto-split-tokenizer-优于-group-wise-tokenizer)
- [# 8.2 timestamp-aware fusion 更优](#82-timestamp-aware-fusion-更优)
- [# 8.3 `[SEP]` token 有帮助](#83-sep-token-有帮助)
- [# 8.4 NS-token 使用 token-specific 参数更好](#84-ns-token-使用-token-specific-参数更好)
- [# 8.5 Causal attention 是有效折中](#85-causal-attention-是有效折中)
- [# 8.6 Pyramid Stack 降低计算成本](#86-pyramid-stack-降低计算成本)
- [# 10.1 模型层面的价值](#101-模型层面的价值)
- [# 10.2 工程层面的价值](#102-工程层面的价值)
- [# 10.3 业务层面的价值](#103-业务层面的价值)
- [# 11.1 第一阶段：统一 token 化](#111-第一阶段统一-token-化)
- [# 11.2 第二阶段：引入 causal attention 和缓存](#112-第二阶段引入-causal-attention-和缓存)
- [# 11.3 第三阶段：扩展序列长度和模型深度](#113-第三阶段扩展序列长度和模型深度)
