...
用长度表示，某层若只让 \(L'\) 个尾部 token 做 Query，而仍保留长度 \(L\) 的历史作为 Key/Value，则注意力主项由近似 \(O(L^2d)\) 变为 \(O(LL'd)\)，FFN 也只在这 \(L'\) 个 Query 位置上计算。Pyramid 的作用是减少深层需要更新的 Query 数，不是把早期历史立即从可读记忆中删除。

## 5.2 Cross-request KV Caching
...
论文还采用 FlashAttention-2、BF16/FP16 混合精度和 activation recomputation。它们不是核心建模模块，但决定这条大 Transformer 路线能否落地：

| 方法 | 作用位置 | 解决的问题 |
| --- | --- | --- |
| FlashAttention-2 | 训练与推理的 attention kernel | 分块计算，减少注意力的显存读写 |
| BF16/FP16 混合精度 | 训练与推理 | 降显存、提高矩阵乘法吞吐 |
| Activation recomputation | 训练 | 前向少存 activation，反向时重算，以计算换显存 |
| KV cache | 推理 | 复用候选无关的历史 S-token K/V |

因此 OneTrans 的工程逻辑是：Pyramid 减少深层需要更新的 Query 数，KV cache 消除相同历史的重复编码，FlashAttention 和混合精度降低剩余 Transformer 计算的实现成本。四者各管一段，不能互相替代。
...
### 6.2.2 Feature-interaction：只替换特征交互模块
...
### 6.2.3 Sequence-modeling：只替换序列建模模块
...
### 6.2.4 Unified framework：OneTrans
...
该结果同时包含业务指标与 p99 延迟，反映的是统一模型和系统优化的整体收益，不能把增益单独归因于某一个模块。
