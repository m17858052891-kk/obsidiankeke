...
#### RTG 怎样从整段日志回填？线上和 AuctionNet 仿真又怎样更新？

这一段先把三个量分开，否则最容易把“已经得到多少分”和“还希望得到多少分”混为一谈：

| 量 | 含义 | 在当前决策时是否已知 |
|---|---|---|
| $S_t$ | 第 $t$ 个窗口**结束后**，截至当前累计得到的业务 score。 | 当前窗口结束后才知道。 |
| $r_t$ | 第 $t$ 个窗口**开始前**送给 GAVE 的 RTG 条件，表示从这一刻开始希望/还可争取的 score。 | 作为模型条件输入。 |
| $S_T$ | 整个 episode 结束后的终局 score。 | 训练日志中可回看；真实线上当前时刻不可见。 |

最关键的一点是：**这里的 $r_t$ 不是单步 reward，也不是刚刚已经拿到的累计 score。**单步增量若需要写，应是 $S_t-S_{t-1}$；GAVE 使用的 $r_t$ 是“从当前窗口开始到结束”的 score-based RTG。

##### 1. 离线训练：整段轨迹结束后，再为每个窗口回填条件

训练日志拥有一整天已经结束的完整轨迹。选定与业务评估一致的 score（论文主实验使用 $S_2$）后，先计算每个窗口边界的累计 score $S_0,S_1,\ldots,S_T$，最后取得终局 $S_T$。对第 $t$ 次决策，回填：

$$
r_t = S_T - S_{t-1}
$$

含义是：模型准备在窗口 $t$ 决策时，离终局还剩多少 score。为了完整表示结束状态，也可记 $r_{T+1}=0$。

用三窗口例子走一遍。假设最终 score 为 100：

| 决策时刻 | 此时已经完成的窗口 | 当时已累计 score | 喂给模型的 RTG 条件 | 执行当前窗口后 |
|---|---|---:|---:|---|
| 窗口 1 开始前 | 无 | $S_0=0$ | $r_1=S_3-S_0=100$ | 得到 $S_1=40$ |
| 窗口 2 开始前 | 窗口 1 | $S_1=40$ | $r_2=S_3-S_1=60$ | 得到 $S_2=70$ |
| 窗口 3 开始前 | 窗口 1--2 | $S_2=70$ | $r_3=S_3-S_2=30$ | 得到 $S_3=100$ |
| episode 结束 | 全部窗口 | $S_3=100$ | $r_4=0$ | 无下一步动作 |

所以训练样本会是“当前状态 $s_t$、过去动作、回填后的 $r_t$，监督当前日志动作 $a_t$”。$r_2=60$、$r_3=30$ 的确利用了未来终局结果，但它们只是完整日志生成的离线条件标签，不会在真实线上直接偷看未来；这和语言模型训练时利用完整句子、生成时只看到前缀是同一类区别。

##### 2. 真实线上：没有 $S_T$，只能先给一个可执行的目标条件

线上窗口 $t$ 尚未结束时，不存在可直接取得的终局 $S_T$，因此不能照搬 $r_t=S_T-S_{t-1}$。论文披露的线上做法是：真实转化稀疏时，以赢得流量的**预期总转化**构造 RTG；推理开始时，将该 campaign **前一天的预期总转化**作为整段 RTG 条件。

这意味着线上输入的不是“我已经知道今天最终会得到多少转化”，而是一个事先设定的 target return：模型据此偏向生成与这个目标规模相匹配的 bid coefficient。论文没有把这一生产线上条件进一步定义成一个通用的逐窗口 planner，因此不能把后面的 AuctionNet 更新规则直接等价为论文披露的所有线上实现。

##### 3. AuctionNet 仿真：用已发生的环境反馈，逐步把 target return 扣减成下一步条件

公开 AuctionNet 参考代码展示的是仿真推理循环。它维护 `states`、`actions`、`rewards`、`timesteps` 和 `curr_score` 序列，并在每个窗口把新的条件追加到 return 序列：

1. **第一个窗口：**令 `eval_curr_score = target_return`，即把事先给定的整段目标作为 $r_1$。
2. **执行动作并推进环境：**GAVE 输出当前动作；AuctionNet 完成拍卖，环境状态更新，因而当前预算、CPA 和累计结果都发生变化。
3. **后续窗口：**代码从 `budget`、`cpa`、更新后的 `state` 和 `pred_all_reward` 计算当前 score，并做 scale 归一化；然后用目标减去这部分已实现/已计入的 score，得到下一个窗口的剩余条件：
...
-
\frac{\mathrm{getScore}(\mathrm{budget},\mathrm{cpa},\mathrm{state},\mathrm{pred\_all\_reward})}
{\mathrm{scale}}
$$

代码变量名 `curr_score` 容易造成误解：从这个减法看，它保存的更接近“**当前剩余 RTG 条件**”，而不是单纯的“当前已获得分数”。如果目标是 100，第一窗口后按当前状态计算已计入 score 为 40，则下一次模型输入应接近 60；再累计到 70 后，下一步条件接近 30。这正好复现训练样本中的 $100\rightarrow60\rightarrow30\rightarrow0$ 轨迹，但每一步只使用仿真环境已经反馈出的状态。

```text
训练：完整日志结束 -> 算终局 S_T -> 回填每个 r_t
线上：先设 target_return -> 模型输出动作 -> 等当前窗口反馈
      -> 用当前状态算已计入 score -> 更新下一步剩余 RTG
```

因此，AuctionNet 的条件更新不是提前知道未来，而是将一开始给定的目标，随已经发生的拍卖结果不断重算为下一步条件。这里 `getScore` 与 `scale` 是公开参考实现中的任务/数值定义；它说明仿真如何维护条件序列，但不是论文正文额外证明的通用线上 RTG 最优更新定理。
