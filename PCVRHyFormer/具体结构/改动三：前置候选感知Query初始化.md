...
历史上曾尝试将 `candidate item_id` 与 `seq_c.fid47` 逐位置做精确相等判断，并手工汇总“匹配次数、最近匹配时间、匹配比例”后作为输出层残差。该方法只回答“用户是否见过同一条候选广告”，无法表达不同 ID 但语义相关的商品关系；约 10.87% 的样本命中，也使无匹配 sentinel 成为极强的缺失模式。

当前前置 bilinear pooling 则让每个历史行为以连续分数参与 Softmax：
...
\mathrm{softmax}
\left(
\frac{(W_mz_{m,t})^\top q_{\mathrm{item}}}{\sqrt D}
\right)
$$

它回答的是“在当前候选条件下，哪条历史更值得读”，而不是“是否存在完全相同的 ID”。历史注释中硬匹配特征从约 `0.826` 回退至 `0.820`；因此当前更强调软语义对齐，而不将 exact match 当作强规则。
