...
- [# 2.1 架构层面对比](#21-架构层面对比)
- [# 2.2 训练默认参数差异](#22-训练默认参数差异)
- [# 4.1 数据来源](#41-数据来源)
- [# 4.2 请求时间特征](#42-请求时间特征)
- [# 4.3 序列时间桶](#43-序列时间桶)
- [# 5.1 RankMixerNSTokenizer](#51-rankmixernstokenizer)
- [# 5.2 CoupledNSTokenizer](#52-couplednstokenizer)
- [# 5.3 UE tokens](#53-ue-tokens)
- [# 5.4 request time token](#54-request-time-token)
- [# 8.1 Sequence Evolution](#81-sequence-evolution)
- [# 8.2 Query Decoding](#82-query-decoding)
- [# 8.3 Query Boosting / RankMixer](#83-query-boosting-rankmixer)
- [# 改动 1：非序列特征从 concat 向 token 化转变](#改动-1非序列特征从-concat-向-token-化转变)
- [# 改动 2：对 int+dense 对齐特征做 element-level 融合](#改动-2对-intdense-对齐特征做-element-level-融合)
- [# 改动 3：引入 UE dedicated tokens](#改动-3引入-ue-dedicated-tokens)
- [# 改动 4：加入 request time token](#改动-4加入-request-time-token)
- [# 改动 5：序列 token 加 time bucket embedding](#改动-5序列-token-加-time-bucket-embedding)
- [# 改动 6：每路序列独立 Query 生成](#改动-6每路序列独立-query-生成)
- [# 改动 7：Query-to-Sequence Cross-Attention](#改动-7query-to-sequence-cross-attention)
- [# 改动 8：RankMixer 替代重型全 token self-attention](#改动-8rankmixer-替代重型全-token-self-attention)
- [# 改动 9：保持 `d_model=64` 而不是 128](#改动-9保持-dmodel64-而不是-128)
- [# 改动 10：固定学习率，移除 cosine scheduler](#改动-10固定学习率移除-cosine-scheduler)
- [# 改动 11：禁用高基数 embedding 重初始化](#改动-11禁用高基数-embedding-重初始化)
- [# 改动 12：跳过超高基数 embedding](#改动-12跳过超高基数-embedding)
- [# 12.1 UE 2 层 MLP](#121-ue-2-层-mlp)
- [# 12.2 时间衰减乘法](#122-时间衰减乘法)
- [# 12.3 FiLM Action-Type Conditioning](#123-film-action-type-conditioning)
- [# 12.4 CosineAnnealingLR](#124-cosineannealinglr)
- [# 12.5 增大 d_model](#125-增大-dmodel)
- [# 14.1 DIN target-aware attention](#141-din-target-aware-attention)
- [# 14.2 Cross-domain pool](#142-cross-domain-pool)
- [# 14.3 Time-decay pool](#143-time-decay-pool)
- [# 14.4 R-Drop](#144-r-drop)
- [# 14.5 EMA](#145-ema)
- [# 14.6 Label smoothing](#146-label-smoothing)
- [# 14.7 InnerTrans + 更长序列](#147-innertrans-更长序列)
- [# 14.8 LAIN length-conditioning](#148-lain-length-conditioning)
- [# 14.9 Explicit item-sequence match features](#149-explicit-item-sequence-match-features)
- [# 14.10 新增模块叠加后的总体问题](#1410-新增模块叠加后的总体问题)
