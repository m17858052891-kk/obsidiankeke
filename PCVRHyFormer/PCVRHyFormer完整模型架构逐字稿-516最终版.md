...
> 本文以 `baseline - 516改` 的最终代码为唯一架构事实来源，目标是让自己能够从输入数据开始，一直讲到输出概率、损失函数与训练策略。
>
> 文中会严格区分三类信息：
>
> 1. **当前 516 配置实际启用的模块**；
> 2. **代码已经实现、但当前启动脚本没有启用的可选模块**；
> 3. **基于结构做出的解释性理解**，这类内容不能冒充消融结论。
...
> 这个项目不是简单把四路序列拼起来喂给 Transformer，而是把静态上下文和动态历史分开建模。四路历史各自演化，候选物品先参与 Query 初始化，让 Query 带着“当前要预测谁”的信息去读历史；不同域的兴趣只在 RankMixer 中交互，最后用 8 个兴趣 Query 做 PCVR 预测。
...
- 静态上下文和动态行为序列分别 Token 化；
- 四路序列先独立建模，保留各域语义；
- 候选信息在 Query 生成阶段就进入，而不是拖到最后；
- 用多个 Query 表达多兴趣，而不是用单一向量强行概括全部历史；
- 跨域融合通过 RankMixer 有控制地发生；
- 时间信息与行为位置对齐后注入。
