...
### 1. 一条曝光的 bid 与一个时段的 action 不是一回事
...
| Eq.(3) | 解析竞价层。 | $\hat a_t$ 与曝光特征。 | $b_i^*$。 | 生成每次曝光最终 bid。 |
...
### 6. Constraint condition：希望轨迹满足什么约束
...
### 7. Feedback condition：希望轨迹具备什么形状
...
## 第四部分：Reverse Process 怎样生成未来状态计划
...
### 12. Classifier-free guidance 怎样起作用？
...
## 第五部分：state plan 怎样变成 action，再变成 bid

### 15. Inverse dynamics $f_\phi$ 的角色
...
得到 $\hat a_t$ 后，可将其中的 $\hat\lambda_0,\ldots,\hat\lambda_J$ 代入 Eq.(3)：
...
随后对于高价值曝光，$v_i$ 较大，bid 会相对更高；对于低价值或约束风险较高的曝光，Eq.(3) 会给出更低 bid。这保留了曝光级细粒度，同时让时段级控制遵循状态计划。
...
因此 AIGB 的代价是推理比单步 policy 更重。它适合时段级决策，不适合每条曝光都跑一次扩散采样。曝光级仍由 Eq.(3) 快速执行。
