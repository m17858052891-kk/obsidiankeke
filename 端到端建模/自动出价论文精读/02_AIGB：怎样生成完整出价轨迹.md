...
| 阶段 | 输入 | 做什么 | 输出 | 是否执行竞价 |
|---|---|---|---|---|
| 离线训练 | 日志中的状态轨迹、条件、动作 | 对状态轨迹加噪并学习去噪；用状态转移学习 inverse dynamics | 去噪网络 $\epsilon_\theta$、inverse dynamics $f_\phi$ | 否 |
| 生成/服务 | 当前历史状态 $s_{0:t}$、目标条件 $y(\tau)$ | 从随机噪声反向去噪，生成未来状态计划 | $x'_0$ 及计划下一状态 $s'_{t+1}$ | 此时还没有直接生成 bid |
| 执行 | 历史状态、计划下一状态、曝光特征 | inverse dynamics 输出时段参数，再套解析公式 | 当前动作 $\hat a_t$、曝光级 $b_i^*$ | 是 |

把它写成一条链路：

```text
离线训练：真实 x0 --forward 加噪--> xk --εθ 学习去噪--> 训练出 εθ
离线训练：真实 (s历史, s下一步, a) --回归--> 训练出 fφ

生成服务：历史 s0:t + 条件 y --reverse 去噪--> 未来计划 x'0
          --取 s't+1 + 历史--> fφ --解析公式--> ât / bi*
```

因此，**forward diffusion 不是生成 bid 的步骤，而是为了训练去噪网络制造“带噪训练题”；真正生成计划的是 reverse diffusion，真正落到 bid 的是 inverse dynamics 加解析竞价公式。**

#### 按 Figure 2 的箭头逐一读


1. **日志轨迹 $\tau$**：一条 episode 中按时间记录的 $(s_t,a_t,r_t)$；从中取出状态序列 $x_0(\tau)$，并计算该轨迹对应的 return、约束和过程反馈。
2. **Forward Process $q$**：把干净轨迹 $x_0(\tau)$ 逐步加噪成 $x_k(\tau)$。这一步只负责制造训练输入，不负责生成任何线上 bid。
3. **噪声预测网络 $\epsilon_\theta$**：接收带噪轨迹、扩散步 $k$ 和条件 $y(\tau)$，学习预测本次加入的噪声；训练完成后，它才具备反向去噪能力。
4. **Reverse Process $p_\theta$**：推理时从随机噪声 $x'_K$ 出发，固定已发生的历史状态 $s_{0:t}$，在条件 $y(\tau)$ 下逐步得到未来轨迹 $x'_0(\tau)$。
5. **计划下一状态 $s'_{t+1}$**：从整段生成轨迹中只取当前要执行的下一状态目标；它是“希望环境下一步变成什么样”，不是动作本身。
6. **Inverse dynamics $f_\phi$**：把历史状态窗口 $s_{t-L:t}$ 和计划下一状态 $s'_{t+1}$ 映射为当前参数动作 $\hat a_t$，其中包含要调整的 bidding parameters。
7. **解析竞价公式**：把 $\hat a_t$ 中的参数与每条曝光自己的 $v_i,c_i,p_{ij}$ 结合，得到曝光级 $b_i^*$，最后才提交给拍卖系统。

这条箭头顺序也解释了为什么 AIGB 不是“扩散模型直接预测 bid”：扩散模型的输出停在状态轨迹，动作由 inverse dynamics 产生，单曝光 bid 由解析层产生。

### 1.2 先明确：AIGB 里的“完整轨迹”是什么？
...
第一部分先回答了“模型生成什么”，这一部分再回答“模型按什么方向生成”。条件不是 forward diffusion 产生的标签，也不是模型自动凭空推断出的业务目标；它来自一条 episode 的收益、约束和过程反馈。离线训练时，条件和日志轨迹一起作为条件生成的监督信息；生成时，则由目标策略或控制模块提供目标条件。

![[AIGB Figure 2 - Conditions.png]]

图中左侧的 $x_K(\tau)$ 是待生成轨迹的噪声起点，下面的 Return、Constraints、Feedbacks 经 Linear 编码后形成条件 $y(\tau)$，共同送入去噪网络。这里的条件只负责“引导生成方向”，并不直接输出动作或 bid。
...
## 3. 训练侧：Forward diffusion 如何制造学习任务

![[AIGB Figure 2 - Forward.png]]

这张裁剪图只保留 Figure 2 的 Forward Process：右侧干净轨迹 $x_0(\tau)$ 沿箭头逐步加噪，依次得到 $x_1(\tau),\ldots,x_K(\tau)$。上方的小状态曲线表示轨迹逐渐被破坏；下方的 Conv 结构属于后续去噪网络的主体，训练时用来学习如何恢复这些带噪轨迹。

### 3.1 图中 Forward Process 的直观含义

从日志中的干净状态轨迹 $x_0(\tau)$ 出发，逐步加入高斯噪声：

$$
x_0\rightarrow x_1\rightarrow\cdots\rightarrow x_K.
$$

最终 $x_K$ 接近纯噪声。这个过程是固定的，主要用于训练；它不生成 bid，也不执行竞价。训练时，模型看到不同噪声程度的 $x_k$，学习如何从被污染的轨迹中恢复出原始状态轨迹。
...
## 4. 生成侧：Reverse diffusion 如何得到状态计划

![[AIGB Figure 2 - Reverse.png]]

这张裁剪图对应 Figure 2 的 Reverse Process：推理从噪声轨迹开始，去噪网络在条件 $y(\tau)$ 下逐步得到更干净的状态轨迹。它和上一节的 Forward Process 方向相反：Forward 是固定加噪，Reverse 是模型学习出来的生成过程。

### 4.1 图中 Reverse Process 的直观含义

推理时从随机噪声 $x'_K$ 开始，在 Return、Constraints、Feedbacks 等条件 $y(\tau)$ 下逐步去噪：

$$
x'_K\rightarrow x'_{K-1}\rightarrow\cdots\rightarrow x'_0.
$$

最终得到整段未来状态轨迹 $x'_0(\tau)$。这里的去噪网络才是训练出来的模型；它根据噪声轨迹、当前扩散步和业务条件，逐步恢复出满足目标倾向的状态计划。随后还要经过 inverse dynamics 将计划中的下一状态转成当前时段动作，再经过解析竞价公式生成曝光级 bid。
...
这个公式表示：**已知当前第 $k$ 步的带噪轨迹 $x_k(\tau)$，以及业务条件 $y(\tau)$，模型预测第 $k-1$ 步较干净轨迹 $x_{k-1}(\tau)$ 的概率分布。**

- $x_k(\tau)$：当前噪声程度较高的状态轨迹；
- $x_{k-1}(\tau)$：去掉一部分噪声后的状态轨迹；
- $y(\tau)$：Return、Constraints、Feedbacks 等生成条件；
- $p_\theta$：由参数 $\theta$ 控制的反向去噪分布。

它不是一次性确定输出一个轨迹，而是先得到一个分布，再从分布中采样：

$$
x_{k-1}(\tau)
\sim
\mathcal N\left(
\mu_\theta(x_k(\tau),y(\tau),k),
\Sigma_\theta(x_k(\tau),k)
\right).
$$
...
例如，$x_3$ 几乎是噪声，模型结合“高 return、CPC 合规”的条件先生成 $x_2$，再由 $x_2$ 生成 $x_1$，最后得到符合条件的 $x_0$。与之相对，Forward Process 的 $q(x_k\mid x_{k-1})$ 是固定的加噪过程，而这里的 $p_\theta(x_{k-1}\mid x_k,y)$ 是模型学习出来的去噪过程。

### 4.2 去噪网络 $\epsilon_\theta$ 做什么？
...
### 4.3 Classifier-free guidance 怎样起作用？
...
## 5. 执行侧：state plan 怎样变成 action，再变成 bid

![[AIGB Figure 2 - Bid Generation.png]]

右侧这张图是生成结果落地的最后一段：生成的状态轨迹 $x'_0(\tau)$ 先进入 inverse dynamics $f_\phi$，得到当前时段参数动作 $\hat a_t$，再代入论文 Eq.(3) 的解析竞价公式，得到单条曝光的 $b_i^*$。
...
从梯度路径看也很清楚：$\mathcal L_{\text{diffusion}}$ 的梯度更新噪声网络参数 $\theta$，$\mathcal L_{\text{ID}}$ 的梯度更新 inverse dynamics 参数 $\phi$；训练时不需要穿过真实拍卖环境，也不会因为一条曝光的最终 bid 再反向改写扩散轨迹。论文将两项监督目标写在同一个总损失中，是为了统一表示 DiffBid 的训练目标，实际理解时仍应把它们看成“轨迹生成器”和“状态到动作执行器”两个职责清晰的模块。
...
### 9.1 AIGB 的仿真系统：RAS 怎样工作？

论文的离线实验不是直接把 DiffBid 放到真实广告流量上反复试错，而是在一个作者手工搭建的 **离线真实广告系统（Real Advertising System，RAS）** 中进行。它的作用是提供一个可重复的多广告主竞价环境：给定广告主、预算、曝光机会和竞价参数，系统运行拍卖、返回赢标/成本/价值等结果，再更新下一时段的状态。

论文明确给出的仿真设定如下：

| 项目 | 论文给出的设置 | 在仿真中的作用 |
|---|---|---|
| 环境 | 手工搭建的离线 RAS | 替代真实线上环境，安全地比较策略。 |
| 环境结构 | 两个连续阶段，拍卖机制参考真实 RAS | 让曝光机会经过连续的竞价/反馈过程；论文没有展开代码级阶段公式。 |
| 一个 episode | 一天 | 所有时间片和曝光结果汇总成一条完整 bidding trajectory。 |
| 时间粒度 | $T=96$，相邻时间片 15 分钟 | 让模型在长时域内规划预算和成本曲线。 |
| 广告主数量 | 30 | 构成多广告主竞争环境。 |
| 每个时间片的曝光机会 | 正文描述为约 100–500；附录 Table 5 列出 $N_{\min}=50,N_{\max}=300$ | 论文正文和表格存在数值口径差异，复现时不能擅自选一个当作唯一官方值。 |
| 预算 | 1000–4000 Yuan | 测试不同预算规模下的泛化。 |
| bid 范围 | 0–1000 Yuan | 限制动作转成出价后的可行范围。 |
| 曝光价值 | 两个阶段的 $v$ 均在 0–1 范围 | 计算赢得曝光带来的 return。 |
| 市场价格 | 最大市场价 1000 Yuan | 与出价比较，决定竞争结果和成本。 |

这里的“仿真”可以按一次 episode 这样理解：

```text
初始化 30 个广告主、预算和当天状态
→ 进入第 t 个 15 分钟窗口
→ 产生该窗口的曝光机会与市场价格/价值
→ 各广告主根据当前状态输出 bidding parameters
→ 通过解析竞价公式得到每条曝光的 bid
→ RAS 执行拍卖，返回是否赢标、成本和价值
→ 汇总本窗口的 spend、return、CPC 等指标
→ 更新剩余预算、消耗速度、CPC、剩余时间等状态
→ 进入 t+1，直到 96 个时间片结束
→ 计算整条轨迹的总 return、约束和评估指标
```

需要特别区分三种数据/环境角色：

1. **行为日志生成**：论文用 USCB 这一已有自动出价方法作为行为策略，在环境中生成离线 bidding logs；基础数据集有 5,000 条轨迹。
2. **扩大动作覆盖**：在 USCB 日志上加入随机探索，得到更有噪声、动作更多样的数据，形成 `USCB-5K`、`USCBEx-5K` 和 `USCBEx-50K` 三个数据集，其中 `USCBEx` 表示加入随机探索。
3. **策略评估**：重新初始化多智能体广告环境，让 USCB 作为基础竞争广告主，DiffBid 和其他 baseline 与其竞争；在预算 1,500、2,000、2,500、3,000 四个档位测试策略表现。

因此，离线仿真评价的是“策略放入 RAS 后，一天 96 个窗口的累计效果”，不是单独评价某一条曝光的 bid 是否准确。它主要检验：

- 生成的状态/动作轨迹能否带来更高总价值；
- 在不同预算下是否仍能保持稳定；
- 约束条件、早晚花和 smoothness 条件是否能控制轨迹形状；
- 在有随机竞争对手和随机曝光机会时，是否比 USCB、BCQ、CQL、IQL、DT 更有效。

论文没有公开 RAS 的完整代码、拍卖匹配细节、市场价格采样公式或状态转移方程；
