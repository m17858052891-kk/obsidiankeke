...
> **一句话总结：Guide 用 Decision Transformer 同时预测下一步出价动作和环境状态，再用 Inverse Dynamics Module 根据预测的状态转移生成一个更贴近历史行为的备用动作，最后由 Twin Q 网络在探索动作和保守动作之间选择。**

## 1. 先用一句话讲清楚

自动出价的难点不是只预测一个 multiplier，而是每隔一段时间都要在“继续探索更高收益策略”和“不要让平台承担危险动作”之间做选择。
...
因此它不是单纯的 DT，也不是单独训练一个逆动力学模型，而是一个 **Explore–Safeguard–Select** 流程：

- **Explore：**DT 通过 Q-value 正则化，尝试比日志动作更高价值的动作；
- **Safeguard：**IDM 根据状态转移重建一个行为一致、波动较小的备用动作；
- **Select：**Twin Q 网络分别评估两个动作，选估计价值更高的一个。

## 2. Abstract：摘要

### 2.1 对应翻译

自动出价已经成为现代数字广告系统的核心组件。早期方法主要基于规则，虽然实现简单，但很难适应快速变化的广告环境。之后的强化学习方法把出价建模为 Markov Decision Process，但对长期依赖关系的捕捉能力有限。近年来的生成式模型取得了不错进展，但通常缺少显式的机制来平衡探索和安全。

已有方法往往只使用简单的动作扰动或轨迹引导来促进探索，而且缺少安全回退机制。当探索进入未知或危险的策略区域时，系统无法回到已知的稳健策略，导致探索效率低，并显著增加广告平台的资金风险。

论文提出 Guide，即 **Generative Auto-Bidding with Unified Modeling and Exploration**。Guide 使用 Decision Transformer，同时建模历史出价动作和环境状态转移；使用 Q-value 模块通过正则化约束引导 DT 探索。与此同时，Inverse Dynamics Module 利用 DT 预测的未来状态，推断与历史行为一致的出价动作，作为安全备用策略。最后，Q-value 模块在 DT 动作和 IDM 动作之间自适应选择，从而同时兼顾探索效率与部署安全。

论文在公开数据集、广告竞价仿真环境和淘宝线上系统中进行了实验。线上 A/B 测试中，Guide 带来：Ad GMV 提升 4.10%，Ad Click 提升 1.40%，Ad Cost 提升 1.66%，Ad ROI 提升 3.52%。

### 2.2 摘要真正要解决的问题

摘要的主线不是“把三个网络堆在一起”，而是针对生成式自动出价中的一个矛盾：

```text
只模仿历史 -> 安全，但不容易超过日志策略
激进探索   -> 可能找到高收益，但探索失败的成本很高
```

Guide 的解决方案是让 DT 负责探索，让 IDM 提供行为一致的 fallback，再由 Q 网络决定当前时刻相信哪一个动作。

## 3. 1 Introduction：引言

### 3.1 传统自动出价为什么不够

论文先从广告系统的业务背景出发：广告主希望在预算、CPA 或 ROI 等目标下获取更高价值的流量。人工出价和早期规则方法难以同时满足实时响应、大规模投放和复杂目标控制。

早期自动出价常使用 PID 等反馈控制方法。它们容易实现，但强依赖参数调节，对竞争环境、流量分布和用户行为变化的适应能力有限。

### 3.2 RL 和生成式模型分别解决了什么

强化学习把出价写成 MDP：当前状态包含预算、历史结果和市场信息，动作是当前出价，策略根据状态选择动作。问题是，MDP 通常把当前状态和当前动作作为主要建模对象，难以充分表达广告环境中的长时间依赖和复杂历史行为。

生成式模型将广告出价改写为序列生成问题，能够利用较长的历史轨迹：

- 基于 DT 的方法主要生成出价动作序列；
- 基于 Decision Diffusion 的方法主要生成广告环境状态序列，再通过逆动力学得到动作；
- GAS、GAVE 等方法进一步加入搜索或价值引导，改善探索能力。

论文认为，这些方法仍然有一个共同缺口：**它们可以探索，但通常没有显式的安全回退机制。**

### 3.3 探索与安全的核心矛盾

广告竞价是高风险场景。每次动作都可能消耗真实预算，因此“无约束地试错”并不适合直接上线。
...
Guide 的基本设计因此是：DT 允许探索，IDM 学习行为轨迹中的可达转移，Q 模块在两个候选动作之间做选择。

### 3.4 Figure 1：不同广告出价建模方式
...
这张图的阅读重点是：不同方法对“状态、动作以及更优动作”的关系建模方式不同。Guide 不仅生成动作，还让 DT 预测下一状态，再用 Q 模块评估不同候选动作。
...
引言完成了从业务问题到模型设计的推导：传统规则适应性不足，RL 的状态建模不够长，生成式模型有探索能力却缺少 fallback，于是需要一个同时建模状态和动作、并能在候选动作间切换的框架。

## 4. 2 Related Works：相关工作

### 4.1 PID 与规则控制

PID 类方法根据预算消耗、CPA 偏差等反馈调整出价系数。优点是结构简单、容易解释，缺点是需要大量人工调参，并且对市场动态变化的适应能力有限。

### 4.2 强化学习自动出价

USCB、SORL 等方法以及 IQL、CQL 等离线 RL 方法，利用广告日志学习策略。它们能够处理自动决策问题，但论文认为这类方法没有充分利用日志中丰富的长历史信息。

### 4.3 生成式自动出价

论文把生成式方法分成两类：

1. **Decision Diffusion：**生成广告环境状态序列，再通过逆动力学生成动作。AIGB 是代表方法，EGDB 进一步加入专家信息优化生成轨迹；
2. **Decision Transformer：**直接生成出价动作序列。GAS 使用 post-training search，GAVE 使用价值引导的动作探索。

论文对这些方法的共同判断是：它们改善了生成和探索，但没有显式处理探索失败后的安全回退。

### 4.4 LLM-based agents

RTBAgent 等工作尝试利用 LLM 的规划能力微调基础出价模型，但仍面临推理延迟高、结果可复现性不足等问题。

### 4.5 广告竞价仿真器

高保真仿真环境能够弥合静态离线数据和动态真实竞价之间的差距。AuctionNet 提供大规模广告竞价数据集和仿真环境；Bid2X 进一步使用 Transformer 环境模型刻画广告竞价动态。

### 4.6 本节作用

相关工作把 Guide 放到四条技术路线中：控制、RL、生成式模型和 LLM。Guide 的定位是：沿用生成式序列建模的长历史能力，同时补上安全 fallback 和候选动作选择。

## 5. 3 Preliminary：预备知识

### 5.1 3.1 Definition of Auto-Bidding Problem：自动出价问题定义

#### 5.1.1 3.1.1 Problem Setting：问题设置

自动出价的目标是在预算和 CPA 约束下，最大化赢得流量的总价值。令 $x_i\in\{0,1\}$ 表示是否赢得第 $i$ 次曝光，$v_i$ 表示该曝光带来的价值，$c_i$ 表示赢得该曝光的实际成本，则问题写成：

$$
\max_{\{b_i\}_{i=1}^{I}}\sum_{i=1}^{I}x_i v_i. \tag{1}
$$

预算约束为：

$$
\sum_{i=1}^{I}x_i c_i\leq B. \tag{2}
$$

CPA 约束为：

$$
CPA=\frac{\sum_{i=1}^{I}x_i c_i}{\sum_{i=1}^{I}x_i v_i}\leq C. \tag{3}
$$
...
#### 5.1.2 3.1.2 Optimal Bidding Policy：最优出价策略

根据互补松弛条件，论文引用已有结果，将理论最优出价写成曝光价值和 CPA 阈值的函数：

$$
b_i^*=(\lambda_0^*+\lambda_1^*C)v_i=\lambda^*v_i. \tag{4}
$$
...
### 5.2 3.2 Sequence Modeling for Auto-bidding Problem：自动出价的序列建模
...
### 6.2 4.1 Unified Modeling of Bid Trajectories：统一建模出价轨迹

#### 6.2.1 4.1.1 Trajectory Construction and Modeling：轨迹构造与建模

广告竞价的一轮过程按时间顺序记录环境状态、出价动作和对应奖励：

$$
\tau=(s_1,a_1,r_1,s_2,a_2,r_2,\ldots,s_T,a_T,r_T). \tag{5}
$$

Guide 让 DT 同时预测下一步动作和下一状态：

$$
(\hat a_t,\hat s_{t+1})\sim DT(R_{t-k+1},s_{t-k+1},a_{t-k+1},\ldots,R_t,s_t). \tag{6}
$$
...
#### 6.2.2 4.1.2 Inverse Dynamics Module：逆动力学模块
...
$$
\hat a_t^{idm}=f_{idm}(s_t,\hat s_{t+1}). \tag{7}
$$
...
\left[\left\|f_{idm}(s_t,\hat s_{t+1})-a_t\right\|^2\right]. \tag{8}
$$
...
#### 6.2.3 4.1.3 Two-Stage Training：两阶段训练
...
$$
\mathcal{L}_{idm}'=\mathbb{E}_{(s_t,a_t)\sim\mathcal{D}}
\left[\left\|f_{idm}(s_t,\operatorname{stop\_grad}(\hat s_{t+1}))-a_t\right\|^2\right]. \tag{9}
$$

DT 单独学习动作行为克隆和下一状态预测：

$$
\mathcal{L}_{dt}=\mathbb{E}\left[(\hat a_t-a_t)^2+(\hat s_{t+1}-s_{t+1})^2\right]. \tag{10}
$$

**阶段二：联合训练。**预训练完成后，放开 DT 预测状态到 IDM 的梯度，使 IDM 的逆动力学误差可以反向影响 DT：

$$
\mathcal{L}=\mathcal{L}_{dt}+\mathcal{L}_{idm}. \tag{11}
$$

两阶段的工程逻辑是：先让两个模块各自学会基本能力，再让它们围绕状态—动作一致性协同优化。这样可以减少训练早期的梯度不稳定，并让 IDM 先形成质量较好的逆动力学表示。

### 6.3 4.2 Q-value-based optimization：基于 Q-value 的优化

仅用离线日志监督，DT 最终容易停留在行为策略附近，难以主动发现更好的动作。Guide 引入 Twin Q critic，用 Q-value 正则化引导 actor，也就是 DT 和 IDM 相关的动作生成模块。

#### 6.3.1 4.2.1 Twin Q Networks and Target Networks Architecture
...
#### 6.3.2 4.2.2 Critic Training Procedure：Critic 训练
...
Q_2^{\text{target}}(s_{t+1},a_{t+1})\right\}. \tag{12}
$$
...
\left(Q_2(s_t,a_t)-y_t\right)^2\right]. \tag{13}
$$

#### 6.3.3 4.2.3 Q-Optimized Actor Training：Q 优化的 Actor 训练

论文将 Q-value 正则化加入 actor 损失：

$$
\mathcal{L}_{actor}=\mathcal{L}_{dt}+\mathcal{L}_{idm}
+\mathbb{E}_{s}\left[-\min\left(Q_1(s,\hat a),Q_2(s,\hat a)\right)\right]. \tag{14}
$$

由于正则项前面有负号，最小化该损失会鼓励 actor 生成更高 Q-value 的动作。因此，模型不是完全复现日志动作，而是在行为监督的基础上，向 Q 估计的高价值方向做探索。

论文有一个必须明确的细节：公式（14）中的 Q 正则化主要引导 DT 的探索动作，**Q 模块不参与 IDM 的训练**。IDM 仍然根据 DT 预测的状态转移，重建日志中体现出的行为动作。

### 6.4 4.3 Q-value Based Action Selection at Inference：推理时的动作选择
...
## 7. 5 Offline Experiment：离线实验

### 7.1 5.1 Experimental Setting：实验设置
...
#### 7.1.2 5.1.2 Metrics：指标

AuctionNet 的 bidding score 用价值和 CPA 惩罚共同计算：

$$
Score=\mathbb{P}(CPA;C)\cdot\sum_i x_i\cdot v_i. \tag{16}
$$
...
$$
\mathbb{P}(CPA;C)=
\min\left\{\left(\frac{C}{CPA}\right)^\beta,1\right\}. \tag{17}
$$

论文通常设置 $\beta=2$。当 $CPA\leq C$ 时，惩罚函数为 1；只有当 CPA 超过约束时才会扣分。

#### 7.1.3 5.1.3 Baselines：基线
...
### 7.2 5.2 RQ1：离线数据与仿真环境结果

#### 7.2.1 AuctionNet 离线评估
...
#### 7.3.1 5.3.1 Ablation Study：消融实验
...
#### 7.3.2 5.3.2 Two-stage Training Analysis：两阶段训练分析
...
### 7.4 5.4 RQ3：DT 和 IDM 的协同

#### 7.4.1 5.4.1 两种动作源的使用偏好
...
## 8. 6 Online A/B Test：线上 A/B 测试

### 8.1 6.1 Deployment：部署方式
...
### 8.2 6.2 Online A/B Test Results：线上结果

#### 8.2.1 6.2.1 Overall Performance：整体表现
...
#### 8.2.2 6.2.2 Trajectory optimization capability：轨迹优化能力
...
## 9. 7 Conclusion and Limitations：结论与局限
...
1. DT 不只预测动作，还预测动作之后的下一状态；
2. IDM 根据当前状态和 DT 的下一状态，反推出一个行为一致的动作；
3. Q 正则化让 DT 有方向地探索，Q 选择让线上执行在两个候选动作中择优；
4. Guide 的核心价值是把“探索能力”和“安全回退”放在同一条出价链路里。

## 11. 与前面几类自动出价方法的定位

| 方法 | 主要生成对象 | 主要增强点 | 约束/安全方式 |
|---|---|---|---|
| DT | 下一步动作 | RTG 条件序列建模 | 主要依赖日志和状态条件 |
| GAVE | 下一步动作 | 价值引导的局部探索 | 软引导，没有显式 fallback |
| AIGB | 未来状态轨迹 | 扩散轨迹规划 + 逆动力学 | 依赖生成轨迹可达性 |
| Guide | 动作 + 下一状态 | IDM fallback + Twin Q 选择 | 行为一致性与 Q 筛选，不是形式化硬约束 |
| GRM | multiplier 响应与约束上界 | 响应预测 + 预算/CPA 求根 | 显式约束求解 |

Guide 相比普通 DT 的关键新增能力，是把“预测下一状态”变成 IDM 的桥梁；相比 AIGB，它用 DT 统一产生动作和下一状态，而不是只生成完整未来状态轨迹；相比 GRM，它重点解决的是探索与 fallback，而不是显式地求解预算和 CPA 的可行上界。

## 12. 原文与图片来源

- 论文 HTML：<https://arxiv.org/html/2605.19457>
- 论文摘要页：<https://arxiv.org/abs/2605.19457>
- 官方代码：<https://github.com/M2C-Tech/GUIDE>
- 本文图片均为论文 HTML 中 Figure 1 到 Figure 7 的独立原图资源。
