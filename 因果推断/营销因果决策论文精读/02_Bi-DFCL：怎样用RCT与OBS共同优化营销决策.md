...
## 15. 与后续 AIGB / GAVE 的关系

Bi-DFCL 原论文不包含 AIGB 或 GAVE。若迁移到乘客券额，它提供的是可信的个体 × 券档结果面：

$$
\{(\Delta\hat r(x_i,a),\Delta\hat c(x_i,a))\}_a.
$$

后续模块若需要：

- AIGB-inspired 宏观规划可将未来预估到达人群的结果面按城市 × 时段聚合为价值池；
- GAVE-inspired 微观策略可将当前乘客的结果面作为离散券档比较与受限探索的价值依据。

具体组合方式见：[[Bi-DFCL + AIGB + GAVE]]。这属于应用层设计，不能说是 Bi-DFCL 论文原生架构。
