...
- [4. Dropout：为什么随机失活能正则化？](#4-dropout为什么随机失活能正则化)
- [5. 高频面试问答](#5-高频面试问答)
- [6. 一张表快速记忆](#6-一张表快速记忆)
...
# 4. Dropout：为什么随机失活能正则化？

Dropout 不是归一化层，而是训练阶段的随机正则化方法。它随机屏蔽一部分激活维度，让模型不能长期依赖某几个固定神经元或特征组合。

## 4.1 训练和推理时分别做什么？

令某层输出为 $h$，dropout rate 为 $p$。训练时采样：

$$
m_i\sim\operatorname{Bernoulli}(1-p),
$$

并采用当前主流框架使用的 inverted dropout：
...
- $m_i=0$：第 $i$ 个激活本次被置零；
- 除以 $1-p$：使 $\mathbb E[\tilde h]=h$，训练与推理的激活期望尺度一致；
- 推理时关闭 Dropout，直接使用完整网络，不再随机置零。

例如 $p=0.2$ 时，一个 batch 中约 20% 的激活会被随机屏蔽；下一 batch 的屏蔽位置又不同。

## 4.2 它为什么能防过拟合？

### 打破 co-adaptation（固定共适应）

没有 Dropout 时，网络可能记住训练集中的脆弱捷径：

```text
特征 A 出现 -> 总依赖神经元 17
神经元 17 出现 -> 总与神经元 42 搭配使用
```

Dropout 使 17 或 42 有时不可用，网络必须让其他维度也能补上信息，因而学习更分散、冗余且稳健的表示，而不是死记一个训练集偶然组合。

### 近似训练大量子网络并在推理时集成

每个随机 mask 都对应原网络的一个子网络。训练中不断切换不同子网络；推理时用完整网络，效果可近似理解为大量共享参数子网络的集成。集成通常降低预测方差，因此更不容易过拟合训练样本。

### 对表示加入噪声，偏好平滑解

同一个样本在不同 mask 下得到的中间表示不同，模型若想稳定预测，就必须满足近似：

$$
f(x;\theta,m_1)\approx f(x;\theta,m_2).
$$

因此训练会偏好对局部神经元失活不敏感的解。可以把它理解为对表示施加随机扰动，降低模型对少数维度的敏感度。

## 4.3 它不是简单“减少参数量”

Dropout 不删除参数，参数量和推理模型大小都不变；它降低的是训练过程中的**有效容量**。每一步只有部分激活参与，模型不能每次都动用全部容量去记忆训练集噪声。

## 4.4 什么时候用，什么时候要谨慎？

- 宽 MLP、小数据、训练验证 gap 明显时，Dropout 往往很有效；
- Transformer 常在 attention、residual 或 FFN 输出上使用较小概率，如 0.05～0.2；具体位置和数值应以模型实现为准；
- 数据量很大时，数据本身已经提供较强正则化，Dropout 可以较小甚至为 0；
- $p$ 太大并非更好：大量信息被随机删除会导致欠拟合、收敛慢；
- 与 BatchNorm 同时使用要谨慎调参：BN 已引入 batch 统计噪声，再加很强 Dropout 有时会过度正则化；
- 不要把普通 Dropout、Embedding Dropout、Attention Dropout、Stochastic Depth 和 MoE Expert Dropout 混为一谈；它们屏蔽的对象不同。

### 30 秒面试回答

> Dropout 在训练时随机置零部分神经元输出，破坏网络对固定神经元组合的共适应，迫使信息分散到更多特征上。每次随机 mask 可以看作训练一个共享参数的子网络，推理时使用完整网络近似于集成，从而降低方差。实现上通常使用 inverted dropout，在训练阶段除以保留概率以保持训练和推理的激活期望一致；推理阶段关闭随机失活。

# 5. 高频面试问答

## 5.1 Transformer 为什么用 LayerNorm，而不是 BatchNorm？
