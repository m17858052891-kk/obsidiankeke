...
> 输入来源：本文假设样本已经按 [[01-特征底座与样本构建]] 中的“事件 → Attempt/宽表 → prediction_time 样本”链路构造完成，再演示它如何进入模型。
...
补充材料将该任务定义为履约/完单预测；具体 label 窗口和正负样本规则仍需以训练配置为准。
