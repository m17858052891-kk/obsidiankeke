...
> **业务闭环边界：** 该模型解决的是履约相关结果的预测与排序；接驾距离即使作为输入特征，也不会因而自动被最小化。若下游 PK/O 选 D 的目标是“履约不显著下降下缩短接驾距离”，仍需要额外的策略层把履约分数、距离和业务约束组合成排序规则。因此，本页的 AUC 结论不能直接推导为 ETP、接驾距离或商家份额收益。
...
它们说明 OneTrans 面向工业长序列的动机。**在本文采用的短序列离线讲解配置中，使用 causal mask，但不启用 Pyramid 动态压缩和 KV cache。**原因是双边序列长度主要在 10/20/30/40 的实验范围内，且当前是离线 batch 训练/验证；Pyramid 与 KV cache 仅作为未来长序列和增量线上推理的扩展能力。这是便于解释的合理配置，不应替代代码事实。
...
### 8.3 由相关 PK 策略背景引出的待验证项

后续 PK 策略材料提供了几个可补强本项目、但**不能反写为当前 OneTrans 已完成**的方向：

1. **条件标签的样本选择偏差。** 若乘客履约仅在“司机未取消/拒单”样本中训练，或司机履约仅在“乘客未取消”样本中训练，训练样本会被对侧行为筛选。当前 D/O/OD 的标签 SQL 未保留，需先核查是否存在该问题；若存在，可将“司机取消、乘客取消、完单”的全样本三分类作为标签建模消融，再由三类概率恢复条件履约率。
2. **AUC 之外的值准。** 当 PK 使用候选中的最高预估履约率作为约束基线时，概率校准会直接影响约束是否可信；因此应补充 logloss、ECE、校准曲线和候选集离线回放，而不仅是 AUC。
3. **预测与决策分层。** OneTrans 验证的是双边序列与当前订单联合交互能否提升离线履约排序；如果业务要约束履约并最小化接驾距离，应在下游策略层显式优化这两个目标，再独立验证完单、CR、ETP、接驾时长和商家份额。

这些补充解释了原始业务目标为何包含 CR、取消率和接驾距离，却不改变本页关于 `pep-seq` 与 `pep-seq-onetrans` 的离线对照口径。
