...
> [!summary] 项目目标
> 每天使用生效日 T 的原策略目标供需补贴率、T-1 日同实验组实际供需补贴率和当前日期类型正在使用的 buffer，按城市和 λ 分组自动校准 `union_buffer`，降低目标供需补贴率与实际供需补贴率之间的偏差。
...
3. 将 BIZ_DATE_LINE 解释为公式观察日 stat_dt=T-1
4. stat_dt 周一～周四选择 weekday；周五～周日选择 weekend
5. 读取当前类型的 union_buffer 作为 old_buffer
6. 关联城市级目标率和城市+LambdaGroup 实际率
7. 计算 new_buffer，并仅按固定上限 2.99 截断
8. 先写 baseline 的 dt=stat_dt 日快照
9. 再覆盖当前 buffer 表的 state_date_type 分区；另一类型不动
10. 最终发布任务另行按 publish_dt=stat_dt+1 读取对应 date_type
```
...
> 初始化与状态更新已合并到一个代码文件；状态更新与最终发布仍是两个阶段。状态更新回答“根据 T-1 观测更新哪套状态”，最终发布回答“T 日线上读取哪套状态”。
...
自动日快照中的 `date_type` 不落结果字段，由分区 `dt=stat_dt` 直接派生。当前 buffer 表以 `date_type` 分区同时保存 weekday/weekend 两套数据；每日计算层由 `stat_dt` 选择唯一 `state_date_type`，只读写该分区。最终发布任务根据 `publish_dt=stat_dt+1` 的日期类型，直接读取当前 buffer 表对应分区。

### 2.3 数据流

```mermaid
flowchart LR
    A[T 日目标补贴率<br/>物理分区 dt=T-1] --> E[自动状态更新]
    B[T-1 实花与平台 GMV] --> E
...
- [x] 已明确分成两步：自动状态表按 `stat_dt=T-1` 的日期类型更新；最终发布按 `publish_dt=T` 的日期类型选择状态。
- [x] 现有人工发布任务确认发布层的分区 `dt=${BIZ_DATE_LINE}` 对应次日 `publish_dt=dt+1` 生效；该语义不改变自动状态更新层的 T-1 规则。
...
3. 自动状态更新由 `stat_dt` 派生 `state_date_type`；
4. T-1 为周一至周四只更新 weekday 状态，T-1 为周五至周日只更新 weekend 状态；
...
9. 边界日允许“本次更新的状态类型”和“本次发布的状态类型”不同。
...
自动日快照通过 `dt=stat_dt` 的星期属性隐式承载本次更新类型，因此无需新增 `date_type` 字段。当前 buffer 表显式以 `date_type` 分区保存两套完整状态，发布层按 `publish_dt` 选择对应分区。

> [!important] 状态表不等于发布快照
> 自动日快照记录“哪一类状态在 T-1 被更新”，当前 buffer 表记录“两类状态目前各自是什么”，最终发布结果记录“T 日线上应该使用哪一类状态”。状态更新和最终发布是两步，不能混成一次简单的最新分区读取。
...
| `state_date_type` | `stat_dt=${BIZ_DATE_LINE}` 自然星期派生 | 决定当日唯一需要更新的状态类型 | 否，由结果 `dt` 反推 |
...
这段代码确认的是**现有人工发布层**的日期语义，而不是自动状态更新层的日期语义。现有发布层在分区 `dt` 中准备 `dt+1` 生效的人工 buffer。

原策略目标率任务写入 `dt=${BIZ_DATE_LINE}`，但从规划源表读取 `dt=${BIZ_DATE_LINE}+1`。现已确认目标补贴率取生效日 T，因此当 `STAT_DT=T-1` 时，直接读取目标表 `dt=STAT_DT`：其物理分区虽为 T-1，业务内容是 T 日目标。
...
```text
stat_dt=周四：更新 weekday 状态
publish_dt=周五：最终发布选择 weekend 状态

stat_dt=周日：更新 weekend 状态
publish_dt=周一：最终发布选择 weekday 状态
```

因此需求中的 T-1 状态更新规则与现有次日发布规则并不冲突，它们属于两个不同阶段。
...
= old_buffer
  ...
  ÷ actual_rate(city_id, LambdaGroup, T-1)
```
...
自动状态更新层目前按需求原文使用 T-1 的周一至周四/周五至周日自然星期分类；是否也需要使用业务日历覆盖，应由业务单独确认，不能直接从人工发布代码推导。
...
合并后的脚本见同目录 `auto_buffer_job.py`。只有在当前 buffer 表完全为空且 baseline 不存在任何历史分区时，才通过城市范围规则展开出与原清单逐键一致的 4067 个组合（weekday 2070、weekend 1997），初值统一为 `1.0`。后续调度发现两个日期分区均存在时自动跳过初始化，直接执行 T-1 状态更新；若只存在一个日期分区则阻断；若当前表为空但 baseline 已有历史，也阻断而不是把线上状态重置为 `1.0`。
...
| 合并自动任务：每日分支 | 每日 | 当前 buffer、原策略目标率、T-1 实际率 | baseline 日快照 + 当前 buffer | 已初始化时跳过首次分支，按 `stat_dt` 日期类型计算并回写本类型状态 |
...
| `date_type` | 任务日期；最终发布可叠加业务日历配置 | `stat_dt` / `publish_dt` | weekday/weekend | 状态更新：`stat_dt` 周一～周四为 weekday，周五～周日为 weekend；发布按 `publish_dt=stat_dt+1` 选类型 | 自然星期逻辑已确认，调休日历待建 |
...
| baseline `target_rate` | `kflower_strategy.platform_price_anchor_union_strategy_budget_dict` | `total_subsidy_rate`；读取 `dt=STAT_DT=T-1`，该分区内容对应规划生效日 T | `city_id` | 单位为百分数值，如 `3.0=3%`；展开到该城市全部生效 `LambdaGroup` | 字段与日期映射已确认 |
...
| `actual_rate` | 上述实花分子 + GMV 分母 | 上游派生字段 | 上游归因到 `dt + city_id + ExpGroupName + LambdaGroup`，向本任务输出 `dt + city_id + LambdaGroup` | `SUM(activity_current_amt) / SUM(gmv_amt) * 100`；分子分母必须使用同一实验流量和城市平台 GMV 口径 | 计算定义已确认；归因链路待确认 |
...
- $buffer^{old}_{i,s,d}$：当前 buffer 状态表中正在使用的 buffer；
- $r_{target}(i,s,T)$：生效日 T 的原策略目标补贴率，取目标表物理分区 `dt=T-1` 中的 `total_subsidy_rate`，单位为百分数值；
...
buffer_{new}(i,s,d)
=
\min\left(2.99, buffer_{old}(i,s,d) \times \frac{r_{target}(i,s,T)}{r^{actual}_{i,s,T-1}}\right)
$$
...
publish_dt   = DATE_ADD(partition_dt, 1)
```
...
| 阶段 | 使用日期 | 日期类型作用 |
|---|---|---|
| 自动状态更新 | `stat_dt=T-1` | 决定更新 weekday 还是 weekend 状态 |
| 最终发布 | `publish_dt=T` | 决定线上当天读取 weekday 还是 weekend 状态 |
...
严格按照需求中的 T-1 规则，先得到唯一 `state_date_type`，再执行一次通用计算链路：
...
- `stat_dt` 为周一至周四，只更新 weekday 状态；
- `stat_dt` 为周五至周日，只更新 weekend 状态；
...
最终发布任务不直接读取自动状态表的最新分区，而是先根据 T 日确定 `publish_date_type`：
...
| `stat_dt` | 状态更新 | `publish_dt` | 最终发布选择 |
|---|---|---|---|
| 周四 | 更新当前 buffer 的 weekday 分区 | 周五 | 当前 buffer 的 weekend 分区 |
| 周五 | 更新当前 buffer 的 weekend 分区 | 周六 | 更新后的 weekend 分区 |
| 周日 | 更新当前 buffer 的 weekend 分区 | 周一 | 当前 buffer 的 weekday 分区 |
| 周一 | 更新当前 buffer 的 weekday 分区 | 周二 | 更新后的 weekday 分区 |

边界日中更新类型和发布类型不同是正常现象，不是状态串用：
...
自动状态更新是否也采用调休后的业务日类型，目前需求没有说明。默认按 T-1 自然星期更新；如果业务希望调休日同时影响状态分类，需要单独确认并复用同一业务日历。
...
partition_dt = BIZ_DATE_LINE
stat_dt = partition_dt
publish_dt = partition_dt + 1 day

state_date_type = (
    "weekday" if stat_dt.weekday() in [0, 1, 2, 3]
    else "weekend"
)
...
现有人工发布任务已经验证发布层的一天偏移。新任务应固定使用 `partition_dt/stat_dt` 表示物理分区与实际观察日，使用 `publish_dt` 表示最终生效日；状态更新使用 `state_date_type`，最终发布使用 `publish_date_type`。目标率表还存在自己的分区偏移，需要按目标实际生效日完成对齐。
...
def resolve_state_date_type(stat_dt):
    pass
...
其中 `target_partition_date=stat_date=T-1`，但该目标表分区的业务内容对应生效日 T；最终发布仍由独立任务按 `publish_dt=T` 完成。
...
- [ ] 确认该看板表的上游完成时间、T-1 分区就绪时间和数据责任人。
...
- [ ] 实现 `stat_dt`、`publish_dt` 和两者各自的日期类型判定。
- [x] 实现 `stat_dt → state_date_type` 唯一选择，当前 buffer 配置按该类型过滤后进入单计算链路。
...
- [ ] 覆盖周四→周五、周日→周一时“更新类型与发布类型不同”的场景。
...
- [ ] `stat_dt=周四` 时更新 weekday，`publish_dt=周五` 时最终发布选择 weekend。
- [ ] `stat_dt=周日` 时更新 weekend，`publish_dt=周一` 时最终发布选择 weekday。
...
已确认的两步日期语义：自动状态表按 `stat_dt=T-1` 更新对应日期类型；最终发布按 `publish_dt=T` 选择对应类型。`partition_dt=stat_dt=${BIZ_DATE_LINE}`、`publish_dt=dt+1`，边界日允许更新类型与发布类型不同。
...
已确认的目标日期语义：目标补贴率取生效日 T，实际补贴率取 T-1。由于目标率任务将规划源 `dt=T` 写入目标表物理分区 `dt=T-1`，自动任务直接读取目标表 `dt=STAT_DT`，不再设置 `TARGET_DT`。
...
19. `activity_current_amt` 是否已经包含退款、冲正和追溯修正，T-1 数据何时达到稳定状态？
20. 已确认 public 日志固定过滤 `subsidy_type=0` 城市补贴率；财务实花分子是否也需要显式排除区县补贴项目？
21. 实验归因后，各实验组 GMV 是互斥拆分还是各自按实验流量计算，是否需要流量膨胀校验？
22. `kflower_strategy.platform_subsidy_city_daily_metrics_di` 的 T-1 分区何时稳定就绪，自动任务能否稳定依赖，数据责任人是谁？
