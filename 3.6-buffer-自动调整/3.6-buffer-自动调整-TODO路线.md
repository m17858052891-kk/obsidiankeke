...
> [!important] 最新目标口径（2026-08-25 更新）
> 目标率为城市粒度；实际率从 `kflower_strategy.pltf_union_stg_budget_control_dashboard` 读取 `act_subsidy_rate`，业务粒度为 `city_id + LambdaGroup`。若实际观测日存在实验切流，受影响的城市—LambdaGroup 本期不做自动调整，直接复制上一期 buffer。

### 0.1 日期与状态序列

```text
STAT_DT = ${BIZ_DATE_LINE}             # 目标表读取分区、baseline 输出分区
T = STAT_DT + 1 natural day            # buffer 业务生效/发布日
STATE_DATE_TYPE = type(T)              # 周一～周四 weekday；周五～周日 weekend
ACTUAL_RATE_DT = T 所属日期类型的上一期
PREVIOUS_BUFFER_DT = ACTUAL_RATE_DT - 1 natural day
```

`date_type` 不落表，由 baseline 的 `dt+1` 唯一推导。weekday 和 weekend 是两条独立状态序列：

| 生效日 T | 日期类型 | `ACTUAL_RATE_DT` |
|---|---|---|
| 周一 | weekday | 上周四 |
...
本文后续“T-1 日实验切流”指本次实际率的观测日 `ACTUAL_RATE_DT=T^-_d`，沿用同日期类型序列，不是固定自然日减一天。

### 0.2 公式输入与粒度

| 输入 | 表 | 日期 | 粒度 | 字段/计算 |
|---|---|---|---|---|
| `old_buffer` | `kflower_strategy.platform_union_strategy_budget_buffer_auto_baseline_by_city_object_stg` | `dt=PREVIOUS_BUFFER_DT` | `city_id + object_group + stg_group` | 上期 `union_buffer` |
| `target_rate` | `kflower_strategy.platform_price_anchor_union_strategy_budget_dict` | `dt=STAT_DT`，内容对应生效日 T | `city_id` | `total_subsidy_rate` |
| `actual_rate` | `kflower_strategy.pltf_union_stg_budget_control_dashboard` | `dt=ACTUAL_RATE_DT` | `city_id + LambdaGroup` | 直接读取 `act_subsidy_rate`；上游公式为 `ROUND((cost + delay_cost) / gmv_amt * 100, 2)` |
| `has_exp_traffic_switch` | `kflower_strategy.pltf_union_stg_exp_evaluation_pas_tag` | `dt BETWEEN PREVIOUS_BUFFER_DT AND ACTUAL_RATE_DT` | `city_id + LambdaGroup` | 同一 `pid` 在窗口内命中多个 `exp_group` 则标记切流，当期复制 `old_buffer` |
...
> [!danger] 实际率上游 SQL 必须先修正
> 参考 SQL 的 `subsidy_by_city_by_exp` CTE 选出了 `lambda_group`，但当前只按 `city_id, exp_group` 聚合。必须改为 `GROUP BY city_id, exp_group, lambda_group`，否则严格 SQL 会报错，即使引擎容忍也无法保证 LambdaGroup 粒度。当多个实验组可能映射到同一 LambdaGroup 时，应先汇总分子、分母后重算比率，不能对 `act_subsidy_rate` 做简单平均。本项目当前以运筹平台的实验组—LambdaGroup 1:1 关系为前提。

`ExpGroupName → LambdaGroup` 的 1:1 映射由运筹平台管理。切流表先产出受影响的 `city_id + exp_group`，再利用实花看板当日的 `exp_group + lambda_group` 关系转为 `city_id + stg_group`；自动 buffer 核心计算层仍只使用 `LambdaGroup/stg_group`。

### 0.3 初始化与上期 buffer

- 某个 `date_type` 第一次运行：根据城市范围和 `stg_group → city_scope` 配置生成键，`old_buffer=1.0`。
- 该类型已有历史：严格读取 `PREVIOUS_BUFFER_DT` 分区的 `union_buffer`。
- 该类型已有历史但严格上一期缺失：任务阻断，不重新回退到 `1.0`。
- weekday 和 weekend 分别只初始化一次，后续都由同类型历史值递推。

初始键配置保留三层压缩结构：`CITY_UNIVERSE_BY_DATE_TYPE`、`CITY_SCOPE_RULES`、`STG_GROUP_TO_CITY_SCOPE`；展开后为 weekday 2070 键、weekend 1997 键。
...
- `has_exp_traffic_switch=1`：不读取当期实际率做调整，`new_buffer=old_buffer`，即在新 `dt` 分区复制一天 buffer。

先判断实验切流，再判断是否执行公式。只有 `has_exp_traffic_switch=0`、`target_rate` 不为空且 `>=0`、`actual_rate` 不为空且 `>0` 时才自动调整。实际率的缺失/零值处理待实际率口径一起确认。
...
- 当前 `dt=STAT_DT` 已存在：直接跳过，避免重跑重复乘调整系数。
- 已存在晚于 `STAT_DT` 的快照：禁止补写旧日期。
- 正常新日期：计算后 `INSERT OVERWRITE` 当前分区。
- baseline 的日分区同时是当日结果、同日幂等快照和后续同类型的状态来源。

当前任务只负责 baseline 自动 buffer，不负责 guardrail 新策略表、实验映射建设、人工/自动组最终合并、线上发布、临时运营补贴率兜底，也不建设独立规则表、当前状态表或审计表。

> [!warning] 当前代码差距
> `auto_buffer_job.py` 已接入实际率和切流优先分支。正式运行前仍需修正实花看板上游 `GROUP BY city_id, exp_group, lambda_group`，并对账确认切流检测窗口为 `PREVIOUS_BUFFER_DT~ACTUAL_RATE_DT`。
...
| 原策略自动 buffer | `kflower_strategy.platform_union_strategy_budget_buffer_auto_baseline_by_city_object_stg` | 唯一输出表；日快照、状态递推来源和重跑幂等结果 |
...
baseline 中的 `date_type` 不落字段，可由 `T=dt+1` 派生。每日计算层由 T 选择唯一类型，并读取该类型上一期对应的 baseline `dt` 分区；最终发布读取本次 `dt=stat_dt` 分区。
...
- [x] 若实际观测日存在实验切流，受影响的 `city_id + LambdaGroup` 不调整，直接复制上一期 buffer。
...
baseline 通过 `T=dt+1` 的星期属性隐式承载本次类型，因此无需新增 `date_type` 字段。weekday/weekend 两条状态链都保存在同一张表的不同时序分区中。
...
|---|---|---|---|
| `date_type` | 由 `T=dt+1` 派生 | 标识 weekday/weekend 两条状态链 | 否 |
...
previous_buffer_dt = actual_rate_dt - 1 day
...
> public 日志可反映当日实际命中的实验组与 LambdaGroup，可作为映射验证和实验切流判断的候选输入。实际率已改为读取实花看板 `act_subsidy_rate`，不能用 public 日志执行补贴率替代。
...
已确认实验组只取 `ExpGroupName`，无需再研究 `Group` 与它是否相等。该映射除影响运筹配置和最终发布外，还需用于上游产出 LambdaGroup 粒度的实际率和切流标记。
...
其中目标率按 `city_id` 关联，实际率和切流标记按 `city_id + LambdaGroup/stg_group` 关联。
...
正式实际率上游（待建）
    → 输出 dt + city_id + LambdaGroup + actual_rate
    → 同时输出 has_exp_traffic_switch
```
...
目标率只有城市粒度，按 `city_id` 展开到全部 LambdaGroup；实际率为 `city_id + LambdaGroup` 粒度，因此同一城市的不同 LambdaGroup 可使用不同调整系数。若 $T^-_d$ 存在实验切流，受影响键不执行上述公式，直接使用 `new_buffer=old_buffer`。
...
→ previous_buffer_dt = T⁻d - 1 natural day
→ 读取 baseline.dt=previous_buffer_dt 的 union_buffer
→ 计算 new_buffer
→ 写 baseline.dt=stat_dt
```

每个日期类型第一次出现时，使用对应键集合和 `old_buffer=1.0` 初始化一次。若某类型已经存在历史，但严格上一期分区缺失，则任务阻断，不能重新回退到 `1.0`。

若 `dt=stat_dt` 已存在，任务直接跳过，不再执行公式。若存在晚于 `stat_dt` 的分区，也禁止旧日期补写；历史回放使用独立影子表按日期顺序递推。
...
最终 baseline 表保持现有物理契约：业务字段为 `city_id, object_group, stg_group, union_buffer, extra_data`，仅以 `dt=stat_dt` 分区。`date_type` 由 `T=dt+1` 派生，不新增字段或分区。
...
计算成功后只写 baseline 当次分区；该表同时是结果、状态历史与幂等结果。
...
public 日志不再用于构建实际率；若用于切流判断，仍需确认切流的标准事件和时间窗口。
...
| `ExpGroupName` | 运筹平台；可用 `kflower_marketplace.ods_log_kflower_platform_call_return_public_log_h` 验证 | 平台/日志 `ExpGroupName`；实花看板和 PAS 切流表中为 `exp_group` | 实验组 | 上游实花按 `exp_group + lambda_group` 聚合；切流判定先定位 `city_id + exp_group`，再转为 LambdaGroup | 已接入 |
...
| `old_buffer` | baseline 同 `date_type` 上一期分区 | `union_buffer` | `previous_buffer_dt + city_id + object_group + stg_group` | 每种类型首次为 1.0；后续严格读取上一期，缺失时阻断 | 已确认 |
...
| `actual_rate` | `kflower_strategy.pltf_union_stg_budget_control_dashboard` | `act_subsidy_rate`；读取 `dt=actual_rate_dt=T^-_d` | `dt + city_id + LambdaGroup` | `ROUND((cost+delay_cost)/gmv_amt*100, 2)`；与 baseline 按 `city_id + stg_group` 关联 | 代码已接入，待上游聚合修正 |
| `has_exp_traffic_switch` | `kflower_strategy.pltf_union_stg_exp_evaluation_pas_tag` | `dt BETWEEN previous_buffer_dt AND actual_rate_dt` | `city_id + LambdaGroup` | `COUNT(DISTINCT exp_group)>1` 的 PID 命中键标记为 1，`new_buffer=old_buffer` | 已实现，待数据对账 |
...
| 自动状态输出 | `kflower_strategy.platform_union_strategy_budget_buffer_auto_baseline_by_city_object_stg` | `city_id`, `object_group`, `stg_group`, `union_buffer`, `extra_data`, `dt` | `dt + city_id + object_group + stg_group` | 唯一写入表；作为当日结果、后续状态来源和同日幂等结果 | 表待创建 |
...
上游 subsidy_by_city_by_exp 必须按 city_id, exp_group, lambda_group 聚合。

kflower_strategy.pltf_union_stg_exp_evaluation_pas_tag
    dt between previous_buffer_dt and actual_rate_dt
    pid
    exp_group
    city_id

同一 pid 的 count(distinct exp_group) > 1
→ 得到受影响 city_id + exp_group
→ 用实花看板 exp_group + lambda_group 转换为 city_id + stg_group
→ has_exp_traffic_switch = 1

按 city_id + LambdaGroup/stg_group 关联 baseline 上一期键
→ 切流键复制 old_buffer
→ 非切流键才使用 actual_rate 计算
```
...
5. 在 previous_buffer_dt~actual_rate_dt 窗口按 PID 判断切流，转换成 city_id + LambdaGroup 标记
6. 上一期 baseline 的 union_buffer 作为 old_buffer
7. 若 has_exp_traffic_switch=1，new_buffer=old_buffer
8. 否则 raw_factor = target_rate / actual_rate
9. 非切流键 new_buffer = MIN(2.99, old_buffer × raw_factor)
10. 写入 baseline 的 stat_dt 分区；若该分区已存在则跳过，避免重复调整
...
```text
IF T^-_d 日受影响的 city_id + LambdaGroup 存在实验切流:
...
每种日期类型第一次出现时以 1.0 初始化一次。已有同类型历史后，若严格上一期分区缺失则任务阻断，不重新初始化。

同日重跑时先检查 baseline 的 `dt=stat_dt` 分区：已存在则直接跳过，不重新执行公式；不存在才计算并写入。
...
dt between previous_buffer_dt and actual_rate_dt
count(distinct exp_group) by pid > 1
has_exp_traffic_switch          # 1=观测窗口存在实验切流
```

实际率的业务键为 `dt + city_id + lambda_group`。本任务使用 `city_id + stg_group` 关联，不能把城市率直接复制给全部 LambdaGroup。看板同时包含全国、城市汇总和实验明细行，自动任务只取后者。正式接入前必须将上游 CTE 修正为 `GROUP BY city_id, exp_group, lambda_group`。

切流判断必须在公式之前：`has_exp_traffic_switch=1` 时，当期直接输出 `old_buffer`，相当于复制一天 buffer；只有非切流键才使用实际率调整。
...
| 同类型上一期 baseline 分区缺失 | 已有同类型历史时阻断；不重新初始化为 1.0 |
| `old_buffer` 非法 | 阻断任务并报警 |
...
- [ ] 修正实际率看板 SQL：`subsidy_by_city_by_exp` 增加 `lambda_group` 到 `GROUP BY`，确保 city + exp_group + LambdaGroup 粒度。
- [x] 切流数据源确认为 `kflower_strategy.pltf_union_stg_exp_evaluation_pas_tag`；同一 PID 在 `previous_buffer_dt~actual_rate_dt` 窗口命中多个实验组即判定切流。
- [x] 切流处理规则确认为：受影响的 `city_id + LambdaGroup` 不做调整，`new_buffer=old_buffer`。
...
- [/] 实际率上游表已确认；待修正 LambdaGroup 聚合并按 `city_id + stg_group` 接入 baseline。
...
- [x] 已明确 baseline 不持久化 `date_type`，由 `T=dt+1` 推导唯一 `state_date_type`；每日只执行一次通用状态更新链路。
...
- [x] 每个日期类型首次出现时以 1.0 初始化一次；已有同类型历史后严格读取上一期，缺失时阻断而不重置。

### Phase 2：计算任务

- [x] 实现 `target_business_dt=publish_dt=stat_dt+1` 的日期类型判定，以及同类型上一期 `actual_rate_dt` 推导。
- [x] 实现 `target_business_dt → state_date_type` 唯一选择，并读取 baseline 同类型上一期进入单计算链路。
...
- [x] 已接入 `has_exp_traffic_switch`，在 buffer 公式前实现切流键 `new_buffer=old_buffer`。
- [ ] 实现城市级运营口径对账指标。
...
- [ ] GMV 为零时保持旧值并报警。
- [ ] 目标率缺失时保持旧值。
- [ ] 同类型上一期 baseline 分区缺失时阻断任务并报警。
- [ ] `T=周五` 时更新并发布 weekend，实际率读取上周日。
- [ ] `T=周一` 时更新并发布 weekday，实际率读取上周四。
- [ ] $T^-_d$ 发生实验切流时，受影响的 `city_id + LambdaGroup` 结果与上一期 buffer 完全一致。
...
baseline 的 `date_type` 由 `T=dt+1` 唯一推导，不参与物理主键。额外校验 old_buffer 只读取严格同类型上一期，且最终不得产生重复业务键。
...
- [ ] 实际观测日发生实验切流时，受影响的城市—LambdaGroup 结果严格等于上一期 buffer。
...
已确认的分层语义：`ExpGroupName → LambdaGroup` 的 1:1 映射由运筹平台管理；`union_stg_v3_max_order_v1_only_box → pltf_union_stg_v3_only_ds_acc_cr_thre` 只是一条示例。自动 buffer 计算层使用 `stg_group=LambdaGroup`。城市级目标率按 `city_id` 展开；实际率和切流标记必须以 `city_id + LambdaGroup` 粒度产出。
...
已确认的切流规则：若实际观测日 $T^-_d$ 有实验切流，受影响的 `city_id + LambdaGroup` 当期不调整，直接复制上一期 buffer。
...
19. 已确认实际率表和公式；待修正上游 `GROUP BY city_id, exp_group, lambda_group` 后验证 `dt + city_id + LambdaGroup` 唯一粒度。
22. LambdaGroup 级实际率分区何时稳定就绪，数据责任人是谁？
23. 已按 PID 跨实验组判定切流，待对账确认 `begin=previous_buffer_dt`、`end=actual_rate_dt` 的日任务窗口与线上口径一致。
