...
```text
STAT_DT = ${BIZ_DATE_LINE} = T         # 目标表分区、baseline 分区和业务生效日
STATE_DATE_TYPE = type(T)              # 周一～周四 weekday；周五～周日 weekend
PREVIOUS_STATE_DT = T 所属日期类型的上一期 # old_buffer 和 actual_rate 共用
...
| 输入 | 表 | 日期 | 粒度 | 字段/计算 |
|---|---|---|---|---|
| buffer 键空间 | `kflower_strategy.platform_union_stg_lambda_dict_manually_update` | 当前表 | `city_id + object_group + stg_group` | `distinct city_id × max_order_v1 × distinct stg_group`；不使用字典表的 `object_group`、`subsidy_rate`、`lambda`或 `is_weekday` |
...
| `target_rate` | `kflower_strategy.platform_price_anchor_union_strategy_budget_dict` | `dt=STAT_DT=T` | `city_id` | 当天 `total_subsidy_rate` |
| `actual_rate` | `kflower_strategy.pltf_union_stg_budget_control_dashboard` | `dt=PREVIOUS_STATE_DT` | `city_id + LambdaGroup` | 直接读取 `act_subsidy_rate`；上游公式为 `ROUND((cost + delay_cost) / gmv_amt * 100, 2)` |
...
`target_rate` 按 `city_id` 左关联并展开到该城市全部 `stg_group/LambdaGroup`；`actual_rate` 使用 `city_id + stg_group` 关联，其中 `stg_group = lambda_group = LambdaGroup`。读取时将看板表的字符串 `city_id` 转为 `BIGINT`，排除全国、城市汇总和未分组行，只保留实验明细行。因此同一城市下不同 LambdaGroup 可以有不同的实际率和调整系数。`act_subsidy_rate` 已是百分数值（如 `3.0=3%`），与目标表单位一致。

> [!important] 实际率上游聚合已修正
> `subsidy_by_city_by_exp` 已按 `city_id, exp_group, lambda_group` 聚合，LambdaGroup 粒度已闭环。结合运筹平台已确认的实验组—LambdaGroup 1:1 关系，baseline 任务可以从实际率表按 `city_id + stg_group`（`stg_group=lambda_group`）关联。
...
- 从 `platform_union_stg_lambda_dict_manually_update` 取 `distinct city_id` 作为城市全集。
- 从同一张表取 `distinct stg_group` 作为 LambdaGroup 全集。
...
ELSE IF target_rate IS NULL OR actual_rate IS NULL OR actual_rate = 0:
    new_buffer = old_buffer
ELSE:
...
- 两者相等：buffer 不变；
- `target_rate=0` 且实际率有效：按公式得到 `new_buffer=0`；
...
`reset_previous_buffer_for_dry_run.py` 是一份完整的空跑计算脚本。它复用正式任务的键空间、目标率 `T`、实际率 `T^-d`、自然日 `T-1` 切流判断、上限和异常处理，但不从 baseline 读取历史 `union_buffer`，而是在当天计算 DataFrame 内将全部键的 `old_buffer` 固定为 `1.0`。

当天空跑计算等价于：

```text
new_buffer(T) = min(2.99, 1.0 × target_rate(T) / actual_rate(T^-d))
```

切流、目标率缺失或实际率缺失/为 0 时，当日仍输出 `1.0`。脚本只 `INSERT OVERWRITE dt=T`，不会覆盖 `PREVIOUS_STATE_DT`，因此不同日期的空跑试算结果能够保留；只有同日重跑会覆盖当天分区。正式上线后必须停止该脚本，改为运行 `auto_buffer_job.py` 以恢复真实 buffer 的连续状态更新；空跑结束后再执行第 0.6 节的一周重置，然后启动正式调度。

> [!warning] 上线前仍需确认
> `platform_union_stg_lambda_dict_manually_update` 必须代表当前有效城市和 LambdaGroup 值域；它没有 `dt/enabled` 字段，若表内保留已失效分组，`distinct stg_group` 会将其继续加入键空间。同时确认 PAS 的自然日 `T-1` 分区和实际率的 `T^-_d` 分区在任务运行前已就绪。
...
- [x] 实际率正式来源为 `kflower_strategy.pltf_union_stg_budget_control_dashboard.act_subsidy_rate`，分子为 `cost+delay_cost`，分母为 `gmv_amt`，单位为百分数值。
...
目标补贴率取生效日 T，本任务直接读取目标率表 `dt=STAT_DT=T`，不关心该表内部的上游加工日期。
...
| 合并自动任务：每日分支 | 每日 | baseline 同类型上一期、T 日目标率、$T^-_d$ 实际率 | baseline 当次分区 | 按 T 的日期类型递推新 buffer |
...
|---|---|---|---|---|---|
| `city_id` 配置键 | 类型首次运行时由脚本范围字典生成；后续来自 baseline 同类型上一期 | `city_id` | 城市 | 原模拟 buffer 被忽略；每种类型第一次以 1.0 初始化，之后沿 baseline 分区递推 | 已实现 |
...
| `LambdaGroup` | 类型首次运行时来自脚本 stg 映射；后续来自 baseline | baseline `stg_group` | λ 分组 | `LambdaGroup = stg_group`；同时是 buffer 键和实际率粒度 | 已确认 |
...
| `actual_rate` | `kflower_strategy.pltf_union_stg_budget_control_dashboard` | `act_subsidy_rate`；读取 `dt=actual_rate_dt=T^-_d` | `dt + city_id + LambdaGroup` | `ROUND((cost+delay_cost)/gmv_amt*100, 2)`；与 baseline 按 `city_id + stg_group` 关联 | 代码已接入，上游聚合已修正 |
...
kflower_strategy.pltf_union_stg_budget_control_dashboard
    ...
    lambda_group AS stg_group
    act_subsidy_rate AS actual_rate

只保留实验明细行，排除 city_id in ('all', 'all2')、
exp_group='all'、lambda_group in ('all', '未分组')。
上游 subsidy_by_city_by_exp 已按 city_id, exp_group, lambda_group 聚合。
...
2. 首次从范围字典生成键和 old_buffer=1.0；否则读取 baseline 上一期分区
3. 城市级 target_rate 展开到每个 LambdaGroup
4. 从实花看板读取 city_id + LambdaGroup 粒度 `act_subsidy_rate`
5. 读取自然日 T-1 分区，按 PID 判断当天是否切流，生成全局单值标记
6. 上一期 baseline 的 union_buffer 作为 old_buffer
...
- $r_{target}(i,T)$：生效日 T 的城市目标补贴率，取目标表物理分区 `dt=STAT_DT` 中的 `total_subsidy_rate`，单位为百分数值；
...
buffer_{new}(i,s,d)
=
\min\left(2.99, buffer_{old}(i,s,d) \times \frac{r_{target}(i,T)}{r^{actual}_{i,s,T^-_d}}\right)
$$
...
当前 PySpark 不实现额外异常兜底表。无切流时直接执行 `MIN(2.99, old_buffer × target_rate / actual_rate)`；目标率、实际率的完整性和可计算性由上游保证。同类型上一期 baseline 分区不存在时，使用 `1.0` 初始化。
...
- [x] 实际率来源确认为 `kflower_strategy.pltf_union_stg_budget_control_dashboard.act_subsidy_rate`，上游公式为 `(cost+delay_cost)/gmv_amt*100`。
...
- [x] 目标率输入契约仅依赖 `budget_dict.dt=T` 的 `city_id + total_subsidy_rate`；上游规划 SQL 不属于本任务的实现或待确认项。
- [ ] 完成目标率、实花、GMV 三方口径对账。
...
- [x] 已接入 `pltf_union_stg_budget_control_dashboard.act_subsidy_rate`，并按 `city_id + LambdaGroup` 关联；不再使用城市日指标表重算实际率。
...
已确认的两步日期语义：`partition_dt=stat_dt=${BIZ_DATE_LINE}=T=publish_dt` 同时是 baseline/目标表物理分区和业务生效日。自动状态更新和最终发布都按 T 选择同一日期类型；实际率与 `old_buffer` 使用该日期类型的上一期，边界日不跨类型读取自然日昨天。
...
已确认的目标日期语义：目标补贴率取生效日 T，实际补贴率取同一日期类型上一期 $T^-_d$。自动任务直接读取目标率表 `dt=STAT_DT=T`，不追溯其上游 SQL，也不另设 `TARGET_DT`。

已确认的补贴率粒度：目标率为 `city_id`，实际率为 `city_id + LambdaGroup`。实际率读取 `kflower_strategy.pltf_union_stg_budget_control_dashboard.act_subsidy_rate`，公式为 `ROUND((cost+delay_cost)/gmv_amt*100, 2)`。
